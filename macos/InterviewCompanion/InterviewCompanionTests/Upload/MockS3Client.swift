...
final class MockS3Client: S3ClientProtocol, @unchecked Sendable {
  ...
  func uploadPart(input: UploadPartInput) async throws -> UploadPartOutput {
    ...
    // Track part upload (data extraction not supported in newer AWS SDK)
    // The ByteStream type doesn't support pattern matching in SDK 1.5+
    uploadedParts.append((input.partNumber ?? 0, Data()))  // Track part number only
