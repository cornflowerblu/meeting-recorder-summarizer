...
final class ConsentAndIndicatorUITests: XCTestCase {
    ...
    func testConsentAcceptanceAllowsRecording() throws {
        ...
        // Find the dialog's Start Recording button (the disabled one that should now be enabled)
        let allStartButtons = app.buttons.matching(identifier: "Start Recording")
        var dialogButton: XCUIElement?
        for i in 0..<allStartButtons.count {
            let button = allStartButtons.element(boundBy: i)
            if button.exists && button.isEnabled {
                dialogButton = button
        ...
        XCTAssertNotNil(dialogButton, "Dialog start button should be enabled after consent")
        dialogButton!.tap()
    ...
    private func startRecordingWithConsent() {
        ...
        guard tapped else {
            XCTFail("Could not find enabled Start Recording button in dialog")
            return
        }
