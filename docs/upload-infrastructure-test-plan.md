...
2. **Test Data**
   ...
   - **Test chunk generator scripts** (see `scripts/README.md`)
     - Python, JavaScript, or Swift versions available
     - Run `./scripts/generate-test-chunks.py` to create test chunks
...
3. **Build Environment**
   ...
   - Python 3 or Node.js (for test chunk generation)
...
Before running manual tests, generate test video chunks using the provided scripts:
...
# Or specify custom count and size
./scripts/generate-test-chunks.py 10 10  # 10 chunks × 10MB each
```

**Available Scripts** (choose one):
- **Python**: `./scripts/generate-test-chunks.py` ⭐ **Recommended**
- **JavaScript**: `node scripts/generate-test-chunks.js` (requires Node.js)
- **Swift**: `swift scripts/generate-test-chunks.swift`

**Output Location**: `~/Library/Caches/MeetingRecorder/test-rec-001/`
